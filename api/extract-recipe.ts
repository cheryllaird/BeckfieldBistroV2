...
async function callGeminiWithRetry(genAI: GoogleGenerativeAI, parts: (string | Part)[]): Promise<string> {
  ...
  console.log('gemini-flash-latest attempt 1/1');
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : String(err);
    console.error(`gemini-flash-latest failed: ${message}`);
    if (!isRateLimitError(err)) throw err;
    primaryErr = err;
  }

  // Primary quota hit — fall back to gemini-flash-lite-latest (separate quota bucket).
  console.log('gemini-flash-latest rate-limited, falling back to gemini-flash-lite-latest');
  try {
    const fallback = genAI.getGenerativeModel({ model: 'gemini-flash-lite-latest', systemInstruction: SYSTEM_PROMPT });
    return await fallback.generateContent(parts).then((r) => r.response.text());
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    console.error(`gemini-flash-lite-latest failed: ${message}`);
    throw primaryErr;
  }
}
